...
PetscErrorCode ThermoElasticity::CreateElasticityNullSpace(DM dm, PetscInt origField, PetscInt field, MatNullSpace *nullspace)
{
  PetscFunctionBegin;
...
void ThermoElasticity::AddMaterial(Material &mtrl)
{
  this->materials.push_back(mtrl);
}

PetscInt ThermoElasticity::GetNumofMaterials()
{
  return this->materials.size();
}

PetscErrorCode ThermoElasticity::SetupAuxDM(DM dm, PetscInt NfAux, PetscFE feAux[], PetscScalar time, PetscInt step)
{
  DM dmAux, coordDM;
  PetscInt f;
  Vec *vec_sol;
...
PetscErrorCode ThermoElasticity::SetupAuxDM(DM dm, PetscInt NfAux, PetscFE feAux[], PetscScalar time, PetscInt step, Vec *sol)
{
  DM dmAux, coordDM;
  PetscInt f;
  ...
  PetscCall(DMSetCoordinateDM(dmAux, coordDM));
  for (f = 0; f < NfAux; ++f)
    PetscCall(DMSetField(dmAux, f, NULL, (PetscObject)feAux[f]));
  PetscCall(DMCreateDS(dmAux));
...
PetscErrorCode ThermoElasticity::SetupMaterial(DM dm, DM dmAux, PetscScalar time, PetscInt step, Vec *sol)
{
  Vec A;
  DMLabel material;
  ...
  for (id = 1; id < numofmaterial + 1; id++)
  {
    ...
    {
      if (j == HEAT_SOURCE_TM) // heat source 13
      {
        ...
        else
        {
          ...
          pointfuncs_user[j] = Pw_Functions::property_user_TM;
          matFuncs[j] = Pw_Functions_common::material_property_nochange;
        }
      }
...
PetscErrorCode ThermoElasticity::CreateDerivedField(DM dm, DM *dmAux, const char name[], PetscFE *fe, PetscFE *fe_derived, PetscInt numComp, PetscInt dim)
{
  PetscBool simplex;
  DM coordDM;
...
PetscErrorCode ThermoElasticity::SolveDerivedField(DM dm, DM *dmAux, PetscPointFunc func, Vec *sol, Vec *derivedsol, const char name[])
{
...
PetscErrorCode ThermoElasticity::ParameterSetup(PetscDS ds)
{
...
PetscErrorCode ThermoElasticity::SolveDerivedFields(DM dm, DM &dmAux, Vec *sol, Vec *derivedsols)
{
  PetscFunctionBeginUser;
  PetscFunctionReturn(0);
}
