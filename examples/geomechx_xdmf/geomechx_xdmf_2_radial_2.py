...
class Xdmf:
  def __init__(self, filename):
    ...
    if sys.version_info[0] < 3:
      ...
      self.typeExt = {2 : {b'vector' : ['x', 'y'], b'tensor' : ['xx', 'yy', 'xy'], b'scalar' : ['radial', 'tangential', 'rtheta', 'zz']}, 3 : {b'vector' : ['x', 'y', 'z'], b'tensor' : ['xx', 'yy', 'zz', 'xy', 'yz', 'xz'], b'scalar' : ['radial', 'tangential', 'rtheta', 'zz', 'none', 'none']}}
    return
...
''' % (f[0]+'_'+ext, domain, cdims, len(f[1].shape), start, stride, size, dims, name))
  ...
  def writeField(self, fp, numSteps, timestep, cellDim, spaceDim, name, f, domain):
    ...
    elif f[1].attrs['vector_field_type'] == b'scalar' and len(f[1].shape) > 1:
      self.writeFieldComponents(fp, numSteps, timestep, spaceDim, name, f, domain)
    else:
